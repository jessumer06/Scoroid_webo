...
html+="<select  class = 'form-control' id = 'sel2' >";
html+="<option>"+"---Select---"+"</option>";
for(var i=0; i<keys.length; i++)
{
    var k = keys[i];
...
var database = firebase.database();
var ref = database.ref('course/'+no+'/ExerciseTable');
...
var ref = database.ref('course/'+no+'/ExerciseTable/'+hell+'/');
...
function gotdi(you,re)
{
    ...
    for(p=0;p<s3-3;p++)
    {
        ...
        var a=p+1;
        var ref8 = database.ref('course/'+no+'/ExerciseTable/'+re+'/'+a+'/');
    ...
    addrow.setAttribute("style","background-color: 5DA2D5;");
    
    addmark.setAttribute("colspan","2");
    adddata.innerHTML="Total Mark:  ";

    addmark.innerHTML="0";
    adddata.setAttribute("style","font-weight: bolder;");
    addmark.setAttribute("style","font-weight: bolder;");
...
function gotLow(good,red,b)
{
    ...
    {
       var t=z+1;
       var ref9 = database.ref('course/'+no+'/ExerciseTable/'+red+'/'+b+'/'+t+'');
